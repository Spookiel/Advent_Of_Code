...
#while the output is not an open space
#Keep sending inputs (4 times max)

#Get the relevant (x,y) position
#Update the current position and board position
import random
moves = [[0,1], [1,0], [-1,0], [0,-1]]
pos = (0,0)

for _ in range(1000000):
    to_face = random.choice(range(4))
    to_check = (moves[to_face][0]+pos[0], moves[to_face][1]+pos[1])
    seen = set()
    while grid.get(to_check,1) == 0 and len(seen)!=4:
        to_face = random.choice(range(4))
        to_check = (moves[to_face][0] + pos[0], pos[1] + moves[to_face][1])
        seen.add(to_check)
    P.input.append(to_face+1)
    ans = P.run()
    grid[to_check] = ans
    if ans==1:
        pos = to_check
...
"""
            try:
                ...
                if o == 0:
                    check = moves[d - 1]
                    walls.add((pos[0] + check[0], pos[1] + check[1]))
                elif o == 1:
                    check = moves[d - 1]
                    new = (pos[0] + check[0], pos[1] + check[1])
                    queue.append(new)
                elif o == 2:
                    check = moves[d - 1]
                    new = (pos[0] + check[0], pos[1] + check[1])
                    print(abs(new[0]) + abs(new[1]))
                    break
