...
data = """dark orange bags contain 3 dark chartreuse bags.
...
def solve(data):
    ...
    for line in data:
        line = line.split()
        first = line[:2]
        cols.add(" ".join(first).strip())
        if "no" in line:
            contains[" ".join(first).strip()] = set()
        else:
            line = " ".join(line[4:]).split(", ")

            for thing in line:
                a = thing[0]
                col = " ".join(thing[1:].split()[:-1])
                #print(col)
                cols.add(col)
                contains[" ".join(first).strip()].add((col, int(a)))
    ...
    for i in range(10):
        for col in cols:
            if not groups[col]:
                ...
                groups[col] = set(thing[0] for thing in contains[col])
            else:
                for item in set(groups[col]):
                    a = groups[item]|set(thing[0] for thing in contains[item])
                    #print(a, item)
                    if groups[item]|set(thing[0] for thing in contains[item]):
                        if item == "shiny gold":
                            continue
                        groups[col] |= a
                        groups[col].remove(item)
    got = 0
    for i in groups.keys():
        #print(i, groups[i], "HERE")
        if "shiny gold" in groups[i]:
            got += 1
    print(got)
