...
def solve(data):
    ...
    for line in lines:
        nums, let, password = line
        nums, let, password = list(map(int, nums.split("-"))), let[:-1],password

        c,d = password[nums[0]-1], password[nums[1]-1]
        if c==let and d!=let or d==let and c!=let:
            count += 1
