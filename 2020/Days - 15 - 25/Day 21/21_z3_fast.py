...
def solve(data):
    ...
    for line in lines:
        ...
        fm.append((a.split(), b))
    allers = list(allers)
    #allers.append("Nothing")
    #print(allers)
    others = list(others)
    #print(others)
    # I want which food maps to which Allergen

    s = Solver()

    # Each food contains at most one allergen
    for ing in others:
        got = Sum([If(Bool(f"{ing}_{allergen}"),1,0) for allergen in allers])
        #print(got)
        s.add(got <= 1)

    for al in allers: #Each allergen is contained by exactly one food
        got = Sum([If(Bool(f"{ing}_{al}"),1,0) for ing in others])
        #print(got)
        s.add(got == 1)

    for fs, als in fm: # One of the foods must be each of the allergens
        for al in als:
            got = Or([Bool(f"{ing}_{al}") for ing in fs])
            s.add(got)

    assert s.check()==sat

    to = defaultdict(lambda: "zzz")
    for i in s.model():
        a,b = repr(i), s.model()[i]
        c,d = a.split("_")
        if b:
            to[c] = d
    ...
    ans = ",".join(filter(lambda y: to[y]!="zzz",sorted(to, key=lambda x: to[x])))
    print(f"P2: {ans}")
