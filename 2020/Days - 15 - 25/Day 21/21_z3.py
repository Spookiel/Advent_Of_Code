...
def solve(data):
    ...
    s = Solver()
    vars = {Int(f"{ing}_{allergy}") for ing, allergy in product(others, allers)}
    for v in vars:
        s.add(0 <= v)
    for v in vars:
        s.add(1 >= v)
    ...
    for ing in others:
        got = Sum([Int(f"{ing}_{allergen}") for allergen in allers])
        #print(got)
        s.add(got <= 1)

    for al in allers:
        got = Sum([Int(f"{ing}_{al}") for ing in others])
        #print(got)
        s.add(got == 1)

    for fs, als in fm: # One of the foods must be each of the allergens
        for al in als:
            got = Or([Int(f"{ing}_{al}")==1 for ing in fs])
            s.add(got)
