...
def compile_rule(r):
    if r == 8:
        return '(' + compile_rule(42) + '+' + ')'
    if r == 11:
        return '(?P<X11>%s(?P&X11)%s|%s%s)' % (compile_rule(42), compile_rule(31), compile_rule(42), compile_rule(31))
    if '"' in rrules[r]:
        return '(' + eval(rrules[r]) + ')'
    res = []
    for chunk in rrules[r].split(' | '):
        x = ''.join(compile_rule(int(c)) for c in chunk.split())
        res.append(x)
    return '(' + '|'.join(res) + ')'
...
count = 0
for row in rows:
    #print(row, regex.match(c, row))
    count += regex.match(c, row) is not None
